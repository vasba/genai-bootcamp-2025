...
class WordReviewService(
    ...
    @Transactional
    fun createWordReview(wordId: Long, sessionId: Long, correct: Boolean): WordReviewItem {
        val word = wordRepository.findById(wordId)
            .orElseThrow { EntityNotFoundException("Word not found with id: $wordId") }
        val studySession = studySessionRepository.findById(sessionId)
    ...
    fun getSessionReviews(sessionId: Long): List<WordReviewItem> {
