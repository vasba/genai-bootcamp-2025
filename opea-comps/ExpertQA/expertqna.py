...
def get_config():
    return {
        "mega_service_port": int(os.getenv("MEGA_SERVICE_PORT", 9999)),
        ...
        "embedding_server_port": int(os.getenv("EMBEDDING_SERVICE_PORT", 80)),  # Changed from EMBEDDING_SERVER_PORT
        "retriever_service_host_ip": os.getenv("RETRIEVER_SERVICE_HOST_IP", "0.0.0.0"),
        "retriever_service_port": int(os.getenv("RETRIEVER_SERVICE_PORT", 7000)),
        "rerank_server_host_ip": os.getenv("RERANK_SERVER_HOST_IP", "0.0.0.0"),
        "rerank_server_port": int(os.getenv("RERANK_SERVICE_PORT", 80)),  # Changed from RERANK_SERVER_PORT
        "llm_server_host_ip": os.getenv("LLM_SERVER_HOST_IP", "0.0.0.0"),
        "llm_server_port": int(os.getenv("LLM_SERVICE_PORT", 80)),  # Changed from LLM_SERVER_PORT
...
class ExpertQnAService:
    def __init__(self, host="0.0.0.0", port=8000):
        ...
        self.endpoint = str("/v1/expertqna")
    ...
    def add_remote_service(self):
        ...
        embedding = MicroService(
            name="embedding",
            host=config["embedding_server_host_ip"],
        ...
        retriever = MicroService(
            name="retriever",
            host=config["retriever_service_host_ip"],
            port=config["retriever_service_port"],
        ...
        rerank = MicroService(
            name="rerank",
            host=config["rerank_server_host_ip"],
            port=config["rerank_server_port"],
            endpoint="/rerank",
            use_remote_service=True,
    ...
    async def handle_request(self, request: Request):
        ...
        return ChatCompletionResponse(model="expertqna", choices=choices, usage=usage)
...
def main():
    config = get_config()
    expertqna = ExpertQnAService(port=config["mega_service_port"])
    expertqna.add_remote_service()
    expertqna.start()
